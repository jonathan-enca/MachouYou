...
function aggregateData(dailyData, granularity) {
  ...
  const buckets = {};
  dailyData.forEach(d => {
    const key = granularity === "week" ? getWeekStart(d.date) : getMonthKey(d.date);
    if (!buckets[key]) {
      buckets[key] = { date: key };
      SUM_FIELDS.forEach(f => buckets[key][f] = 0);
    }
...
function ChartCard({ config, dailyData }) {
  ...
  const granToggle = (
    <div style={{ display: "flex", gap: 2, background: "rgba(255,255,255,0.3)", borderRadius: 6, padding: 2 }}>
      {["day", "week", "month"].map(g => (
        <button key={g} onClick={() => setGran(g)} style={granBtnStyle(gran === g)}>
          {GRAN_LABELS[g]}
        </button>
      ))}
    </div>
  ...
  if (lineOnly) {
    ...
    return (
      <div style={{ background: COLORS.card, borderRadius: 12, border: `1px solid ${COLORS.border}`, overflow: "hidden" }}>
        <div style={{ background: COLORS.headerBg, padding: "6px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ padding: "12px 8px 4px" }}>
          <ResponsiveContainer width="100%" height={160}>
            <LineChart data={lineData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              <Line type="monotone" dataKey="lineVal" name={title} stroke={COLORS.primary} strokeWidth={2.5} dot={{ r: lineData.length > 60 ? 0 : 3, fill: COLORS.primary }} activeDot={{ r: 5 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div style={{ padding: "12px 8px 4px" }}>
        <ResponsiveContainer width="100%" height={200}>
          <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
            ...
            {(line || calcLine) && <Line yAxisId="right" type="monotone" dataKey={lineKey} name={lineLabel || line} stroke={COLORS.line} strokeWidth={2.5} dot={{ r: chartData.length > 60 ? 0 : 2.5, fill: COLORS.line }} />}
          </ComposedChart>
        </ResponsiveContainer>
      </div>
    </div>
...
export default function Dashboard() {
  ...
  const activeData = liveData || INITIAL_DATA;
  const dailyRows = activeData.account_daily;
  ...
  return (
    <div style={{ background: COLORS.bg, minHeight: "100vh", padding: 0, fontFamily: "'DM Sans', -apple-system, sans-serif" }}>
      {/* Header */}
          ...
          <div key={si} style={{ marginBottom: 32 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(340px, 1fr))", gap: 16 }}>
              {section.items.map((chart, ci) => (
                <ChartCard key={ci} config={chart} dailyData={filteredDaily} />
              ))}
            </div>
          </div>
        ))}
